...
export default function SignupforSeller() {
    ...
    const handleSignupforSeller = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post(`${AUTHENTICATION_URL}/Seller/signup`, { name, email, password, bussiness, bussiness_name });
            if (res.data.status) {
            ...
            <div style={{ width: "60%", height: "500px" }} className='container d-flex justify-content-center align-items-center shadow p-0'>
                <div style={{ height: "100%", width: "40%" }} className='bg-primary p-5 d-flex flex-column justify-content-center align-items-center'>
                    <div className='vh-100 w-100'>
                        ...
                        <span className='text-light'>Sign up with your email to get started</span>
                    </div>
                    <div className='mt-3 vh-100 w-100'>
                        <img className='w-100' src="/image/login/loginImage.png" alt="" />
                    </div>
                </div>
                <div style={{ width: "60%", height: "100%" }} className='d-flex flex-column justify-content-between align-items-between px-4 py-3'>
                    <div>
                        <form>
                            ...
                            <div className="form-floating mb-3">
                                <input onChange={(e) => setBussiness(e.target.value)} style={{ border: "none", borderBottom: "2px solid #FB641B", outline: "none" }} type="text" className="form-control" id="floatingInput" placeholder="" />
                                <label htmlFor="floatingInput">Bussiness</label>
                            </div>
                            <div className="form-floating mb-3">
                                <input onChange={(e) => setBussiness_name(e.target.value)} style={{ border: "none", borderBottom: "2px solid #FB641B", outline: "none" }} type="text" className="form-control" id="floatingInput" placeholder="" />
                            ...
                            <div className='d-flex justify-content-center align-items-center my-2'>
                                <button onClick={handleSignupforSeller} style={{ backgroundColor: "#FB641B" }} className="fw-bold text-center text-light btn w-100">Sign Up</button>
                            </div>
                        </form>
                    </div>
                    <div className='d-flex justify-content-center align-items-center'>
                        <Link className="text-center text-primary btn w-100" to="/signin">Existing Seller? Log in</Link>
                    </div>
                </div>
            </div>
        </div>
